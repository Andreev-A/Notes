...
def create():
    ...
    # Подписать пользователей u1 u2 на blog1
    blog1.subscribers.add(u1, u2)
    # Подписать пользователя u2 на blog2
    ...
    # Создать топик title = topic2_content, blog = blog1, author = u3,
    # created = 2017-01-01
...
def edit_u1_u2():
    # Поменять first_name на uu1 у пользователей,
    # у которых first_name u1 или u2
    User.objects.filter(
...
def get_avg_topic_count():
    ...
    return Blog.objects.all().annotate(
        count=Count('topic')).aggregate(avg=Avg('count'))


def get_blog_that_have_more_than_one_topic():
    # Найти блоги, в которых топиков больше одного
    return Blog.objects.all().annotate(
        count=Count('topic')).filter(count__gt=1)
...
def get_user_that_dont_have_blog():
    # Найти пользователей, у которых нет блогов,
