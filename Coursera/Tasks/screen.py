...
if __name__ == "__main__":
    ...
    while working:
        ...
        if screen:
            polyline, knot = polyline_1, knot_1
            steps, change_speed = knot_1.count, polyline_1.change_speed
            screen_name, speed = 'First screen', polyline_1.factor_speed
        else:
            polyline, knot = polyline_2, knot_2
            steps, change_speed = knot_2.count, polyline_2.change_speed
        ...
        if not pause or screen:
            polyline_1.draw_points(polyline_1.points)
            knot_1.draw_points(knot_1.get_knot(), 3, color)
        if not pause or not screen:
            polyline_2.draw_points(polyline_2.points)
