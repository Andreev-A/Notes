...
# Да, хоть предыдущее задание с декоратором и более сложное в реализации, это просто более запутанное без какой-либо
# конкретики и примеров исполнения.
# 1. Начинаем с того, что в классе MappingAdapter инициализируем адаптируемый объект (по аналогии с примером из
# лекции) и в методе lighten получаем сетку из System()
# 2. Задаем размер сетки с помощью метода set_dim. Возвращаем сетку (с помощью другого метода) и отправляем решение.
# Получаем либо ошибку 4 (неправильный размер), либо ошибку 5 (несоответствие целевой матрице). В случае ошибки 4 задаем
# размер матрицы наоборот. Варианта всего 2. В случае ошибки 5 двигаемся дальше
# 3. Когда с размером целевой матрицы определились, начинаем читать матрицу, которая поступила на вход от системы.
# Там уже есть координаты источников света и препятствий. Вот эти координаты надо записать в списки lights и obstacles
# (принимая во внимание "перевернутость")
# 4. Ну, собственно, и все. Теперь расставляем препятствия (!) и источники света (не сами, конечно, для этого уже
# есть их списки и методы для их расставления), ну и возвращаем матрицу с рассчитанными значениями (самим ничего считать
# не надо, все считается в методах)

######################################################################################################################
# Создание адаптера для класса — решение
######################################################################################################################
