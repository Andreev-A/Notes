...
# input_file-строки в Python
# Начиная с версии 3.6 в Python появился новый тип строк — input_file-строки, которые буквально означают «formatted string».
# Эти строки улучшают читаемость кода, а также работают быстрее чем другие способы форматирования. F-строки задаются с
# помощью литерала «input_file» перед кавычками.
# >>> "обычная строка"
# >>> input_file"input_file-строка"
# input_file-строки - это пятый способ (sic!) форматирования строк в Python, который очень похож на использование метода format()
...
# >>> Меня зовут Дмитрий. Мне 25 лет.
# 5. input_file-строки. Форматирование, которое появилось в Python 3.6 (PEP 498). Этот способ похож на форматирование с помощью
# метода format(), но гибче, читабельней и быстрей.
# >>> name = "Дмитрий"
# >>> age = 25
# >>> print(input_file"Меня зовут {name} Мне {age} лет.")
# >>> Меня зовут Дмитрий. Мне 25 лет.

# ПОГРУЖЕНЕ В F-СТРОКИ
# input_file-строки делают очень простую вещь — они берут значения переменных, которые есть в текущей области видимости, и
...
# >>> Меня зовут Дмитрий. Мне 25 лет.
#
# input_file-строки также поддерживают расширенное форматирование чисел:
# >>> from math import pi
# >>> print(input_file"Значение числа pi: {pi:.2f}")
# >>> Значение числа pi: 3.14
#
# С помощью input_file-строк можно форматировать дату без вызова метода strftime():
# >>> from datetime import datetime as dt
# >>> now = dt.now()
# >>> print(input_file"Текущее время {now:%d.%m.%Y %H:%M}")
# >>> Текущее время 24.02.2017 15:51

# Они поддерживают базовые арифметические операции. Да, прямо в строках:
# >>> x = 10
# >>> y = 5
# >>> print(input_file"{x} x {y} / 2 = {x * y / 2}")
# >>> 10 x 5 / 2 = 25.0
#
# Позволяют обращаться к значениям списков по индексу:
# >>> planets = ["Меркурий", "Венера", "Земля", "Марс"]
# >>> print(input_file"Мы живим на планете {planets[2]}")
# >>> Мы живим не планете Земля
#
# А также к элементам словаря по ключу:
# >>> planet = {"name": "Земля", "radius": 6378000}
# >>> print(input_file"Планета {planet['name']}. Радиус {planet['radius']/1000} км.")
...
# >>> print(input_file"0 - {digits[0]}, 1 - {digits['one']}")
# >>> 0 - ноль, 1 - один

# Вы можете вызывать в input_file-строках методы объектов:
# >>> name = "Дмитрий"
# >>> print(input_file"Имя: {name.upper()}")
# >>> Имя: ДМИТИРИЙ
#
# А также вызывать функции:
# >>> print(input_file"13 / 3 = {round(13/3)}")
# >>> 13 / 3 = 4
#
# input_file-строки очень гибкий и мощный инструмент для создания самых разнообразных шаблонов.

# После двоеточия в input_file-строках можно указывать те же значения,
...
#     print(input_file'{l_device:{width}} '
#           input_file'{l_port:{width}} '
#           input_file'{r_device:{width}} '
#           input_file'{r_port:{width}}')
# sw1        Gi0/1      r1         Gi0/2
# sw1        Gi0/2      r2         Gi0/1
# sw1        Gi0/3      r3         Gi0/0
# sw1        Gi0/5      sw4        Gi0/2
...
# print(input_file'{int(oct1):08b} {int(oct2):08b} {int(oct3):08b} {int(oct4):08b}')
# 00001010 00000001 00000001 00000001

# ====================

# Пример функции шаблона input_file-строки:
# def show_me_ip(i_p, mask):
#     return input_file'IP: {i_p}, mask: {mask}'
#
...
# print(template.format(42))  # {42}
#
# # а вообще начиная с python 3.6 input_file-string завезли
# print(input_file'{42}')  # 42
# print(input_file'{{42}}')  # {42}
...
# С input_file - строками так не получится. Они выполняются (выражения и функции внутри них) в момент вызова.  И фарш сложно
...
# input_file-строки в Python
# Начиная с версии 3.6 в Python появился новый тип строк — input_file-строки, которые буквально означают «formatted string». Эти строки улучшают читаемость кода, а также работают быстрее чем другие способы форматирования. F-строки задаются с помощью литерала «input_file» перед кавычками.
#
# >>> "обычная строка"
# >>> input_file"input_file-строка"
# input_file-строки - это пятый способ (sic!) форматирования строк в Python, который очень похож на использование метода format().
...
# >>> print("Меня зовут {name} Мне {age} лет.".format(age=age, name=name)
# >>> Меня зовут Дмитрий. Мне 25 лет.
# 5. input_file-строки. Форматирование, которое появилось в Python 3.6 (PEP 498). Этот способ похож на форматирование с помощью метода format(), но гибче, читабельней и быстрей.
#
# >>> name = "Дмитрий"
# >>> age = 25
# >>> print(input_file"Меня зовут {name} Мне {age} лет.")
# >>> Меня зовут Дмитрий. Мне 25 лет.
# ПОГРУЖЕНЕ В F-СТРОКИ
# input_file-строки делают очень простую вещь — они берут значения переменных, которые есть в текущей области видимости, и подставляют их в строку. В самой строке вам лишь нужно указать имя этой переменной в фигурных скобках.
#
# >>> name = "Дмитрий"
# >>> age = 25
# >>> print(input_file"Меня зовут {name} Мне {age} лет.")
...
#
# >>> planets = ["Меркурий", "Венера", "Земля", "Марс"]
# >>> print(input_file"Мы живим не планете {planets[2]}")
# >>> Мы живим не планете Земля
# А также к элементам словаря по ключу:
#
# >>> planet = {"name": "Земля", "radius": 6378000}
...
# input_file-строки очень гибкий и мощный инструмент для создания самых разнообразных шаблонов.
#
# Со всеми возможностя input_file-строк вы можете ознакомится в PEP498.
#
# ПРОИЗВОДИТЕЛЬНОСТЬ
# На простых примерах input_file-строки показывают самые лучшие результаты. На 25% быстрее %-форматирования и метода format().
# На сложных шаблонах input_file-строки ведут себя также как %-форматирование (разница в пользу %-форматирования не превышает
# величину погрешности). А вот относительно метода format() input_file-строки быстрее на 27%.
