...
# re.A или re.ASCII
# При этом флаге проверки \b, \B, \s, \S, \w и \W действуют так, как если бы они применялись к тексту, содержащему
# только символы ASCII (по умолчанию используется Юникод re.U / re.UNICODE и лучше оставаться в этом режиме)

# re.I или re.IGNORECASE
# Проверка без учета регистра символов
#
# re.M или re.MULTILINE
# Влияет на проверки ^ и $. Начало ^ считается началом строки (сразу после символа \n или начало текста). Конец $
# считается в позиции перед \n (или конец строки)
#
# re.S или re.DOTALL
...
# Все эти флаги достаточно очевидны, кроме, может быть, последнего. Давайте воспользуемся им и запишем наше последнее
# правило с набором комментариев:
...
# Смотрите, мы здесь записали правило в несколько строк, добавили комментарии и объединили два флага: MULTILINE и
# VERBOSE, используя операцию логического ИЛИ.
# Флаги можно указывать и непосредственно внутри выражения, используя синтаксис: (?flags), где flags – один или
...
# print(match)
# Мы здесь включили два флага: re.IGNORECASE и re.MULTILINE. Благодаря первому, в строке находятся все три совпадения
# со словом python:
...
# sw = "r2d2"
# print(1, re.search(r"[a-z]\d[a-z][\da-z]", sw))
# sw = "c3po"
# print(2, re.search(r"[a-z]\d[a-z][\da-z]", sw))
# sw = "luke"
# print(3, re.search(r"[a-z]\d[a-z][\da-z]", sw))
# sw = "f0rce"
# print(4, re.search(r"[a-z]\d[a-z][\da-z]", sw))
# sw = "ev9d9"
# print(5, re.search(r"[a-z]\d[a-z][\da-z]", sw))
# sw = "bb8"
# print(6, re.search(r"[a-z]\d[a-z][\da-z]", sw))


# exp = "м.+?ы"
# print(1, re.findall(exp, "мама вымыла 2 рамы"))
# exp = "....мы"
# print(2, re.findall(exp, "мама вымыла 2 рамы"))
# exp = "м.*ы"
# print(3, re.findall(exp, "мама вымыла 2 рамы"))
# exp = "м.*?ы"
# print(4, re.findall(exp, "мама вымыла 2 рамы"))
# exp = r"а.\D"
# print(5, re.findall(exp, "мама вымыла 2 рамы"))
# exp = "мы"
# print(6, re.findall(exp, "мама вымыла 2 рамы"))
# exp = r"\d+"
# print(7, re.findall(exp, "мама вымыла 2 рамы"))

# def find_all_digit(text):
#     exp = r'\d+'
#     return re.findall(exp, text)
#
# print(find_all_digit('a123b45с6d'))

# ^\W\d{2}\s?\.([abc\d])\1\1$
# ^ и $	Начало и конец строки соответственно
# \W — все, кроме буквы или цифры
# \d	Любая цифра [0-9] {2} - два вхождения
# \s	Любой пробельный символ
# ?	0 или 1 вхождение шаблона слева
# \. означает точку
# [abc\d]- либо a или b или c и Любая цифра [0-9]
# ([abc\d])\1\1 соответствует a или b или c или цифра из первой захватываемой группы три раза
sw = "+13.777"
print(re.search(r"^\W\d{2}\s?\.([abc\d])\1\1$", sw))
